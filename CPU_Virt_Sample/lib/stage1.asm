...
%endif
        ...
        mov WORD [fs: SDA.GdtLimit], 11 * 8 - 1                                  ; 共 11 个 entry        
        mov DWORD [fs: SDA.GdtTop], SDA_PHYSICAL_BASE + SDA.Gdt + 10 * 8         ; top 指向第 10 号 entry  
...
alloc_pcb_base:
        ...
        ;;
        ;; 执行原子 PcbPoolBase 加 PCB_SIZE 操作
        ;;
...
alloc_tss_base:
        ...
        ;;
        ;; TSS 块分配说明：
        ;; 1) 同时分配 virtual base 与 physical base
        ;; 2) 保持 virtual address 与 physical address 对应关系
        ;;
        mov ebx, [fs: SDA.TssPoolGranularity]                   ; TSS 块分配粒度（默认为 100h 字节）
        mov edx, ebx       
        xor esi, esi
...
;       none
; 描述:
;       1) 更新 stag1 的 GS 段，对应处理器的 PCB 区域
;-----------------------------------------------------------------------
update_stage1_gs_segment:
        push edx
        push ecx
...
;       none
; 描述：
;       1) 更新 stage1 的selector（即未分页下的保护模式）
;-----------------------------------------------------------------------
update_stage1_selector:
...
; 描述：
;       1) 构建 TSS 环境，并加载 TR
;       2) 在 stage1（未分页下）使用，此时 TSS 使用物理地址
;-----------------------------------------------------------------------
build_stage1_tss:
        push ebx
        push edx
        ...
        ;;
        ;; 设置 TSS 管理信息
        ;; 1) 地址中的高 32 位，使用在 64-bit 模式下
        ;; 2) TSS limit = 1000h (TSS区域 + IO bitmap 区域)
        ;;
...
build_stage1_tss.@0:
        ...
        mov [ebx + tss32.IomapBase], ax                         ; Iomap 偏移量
